...
class BookFragment : Fragment() {
    ...
    private fun setupSpinner() {
        lifecycleScope.launch {
            ...
            // ตรวจสอบว่า authors มีข้อมูลหรือไม่
            Log.d("DEBUG", "Authors found: ${authors.size}")
            authors.forEach { Log.d("DEBUG", "Author: ${it.name}") }

            if (authors.isEmpty()) {
                Toast.makeText(requireContext(), "No authors found!", Toast.LENGTH_SHORT).show()
                return@launch
            }
            ...
            val adapter = ArrayAdapter(
                requireContext(),
                android.R.layout.simple_spinner_item,
                authors.map { it.name } // แสดงเฉพาะชื่อของ author
            )
    ...
    private fun setupClickListeners() {
        binding.addBookButton.setOnClickListener {
            ...
            val Date = binding.purchaseDateInput.text.toString()
            ...
            if (title.isNotBlank() && yearStr.isNotBlank() && authorPosition != -1) {
                ...
                if (year != null) {
                    addBook(title, year, isbn, authors[authorPosition].authorId, Date)
                    clearInputs()
                } else {
    ...
    private fun addBook(title: String, year: Int, isbn: String, authorId: Long, Date: String) {
        lifecycleScope.launch {
            try {
                repository.insertBook(title, year, isbn, authorId, Date)
                loadBooks()
            } catch (e: Exception) {
    ...
    private fun showEditBookDialog(book: Book) {
        ...
        dialogBinding.editIsbnInput.setText(book.isbn)
        dialogBinding.editPurchaseDateInput.setText(book.Date)
        ...
        MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton("Save") { dialog, _ ->
                ...
                val newIsbn = dialogBinding.editIsbnInput.text.toString()
                val newDate = dialogBinding.editPurchaseDateInput.text.toString()
